...
<p>
$$CAM_c = \sum_{k} {w_k}^{c} \frac{1}{Z} \sum_{i}\sum_{j} F_{(i,j)}^{k}$$
$$CAM_c = \sum_{k} {w_k}^{c} F^{k} $$
</p>
,where $$CAM_c$$ is the activation for $$c$$, which is the specific class (like Australian terrier). $$k$$ is the number of feature maps and therefore $${w_k}^{c}$$ is the weight for feature map $$k$$ for class $$c$$. 
Lastly, $$F_{(i,j)}^{k}$$ is the feature map $$k$$ for pixel coordinate $$(i,j)$$, which is summed over all $$i$$ and $$j$$ and divided by total number of pixels $$Z$$ to return $$F^{k}$$. Finally, since feature maps are downsampled compared to the original image size,
