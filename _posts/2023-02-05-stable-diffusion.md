...
layers. The general idea is that autoregressive models model the joint probability space $$ p(x) $$ by utilizing the chain rule $$ p(x,y) = p(y|x)p(x) $$ More on autoregressive models can be mentioned in later blogs as well.</li>
    <li>3. <strong>Latent variable models:</strong> <strong>Stable diffusion models</strong> belong to this type. Latent variable models are further classified into two categories, explicit and implicit models.
    </li>   
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 1. <strong>Explicit models:</strong> asdfdasf
